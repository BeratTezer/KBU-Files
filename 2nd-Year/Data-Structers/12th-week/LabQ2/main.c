...
void main() {
    pqueue sira1;
    initialize(&sira1);
    printf("Until you type \"-1\" your input will be added to priority queue:\n\n");
    while(1) {
        int input;
        printf("\n-> ");
        scanf("%d", &input);
        if(input==-1) break;
        else insert(&sira1, input);
        printf("\n%d (added)\n", input);
    }
    printf("Until you type \"-1\", each input will remove the first element from the priority queue:\n\n");
    while(1) {
        int input;
        printf("\n-> ");
        scanf("%d", &input);
